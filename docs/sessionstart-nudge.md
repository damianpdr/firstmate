...
| OMP | `.omp/extensions/fm-primary-turnend-guard.ts` handles `session_start` reasons `startup`, `new`, and `resume`, then injects the wrapper output with the Pi-derived `pi.sendMessage` API. | Native context injection is supported through the committed auto-discovered extension. |
